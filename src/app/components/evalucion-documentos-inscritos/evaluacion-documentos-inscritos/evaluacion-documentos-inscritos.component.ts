...
export class EvaluacionDocumentosInscritosComponent implements OnInit {
              ...
              if (response.Success == true && response.Status == 200) {
                ...
                if (i == selectPeriodo.length - 1) {
                  ...
                  this.cantidad_inscritos_obs = this.Aspirantes.filter(
                    (aspirante: any) =>
                      aspirante.Estado == "INSCRITO con Observación"
                  ).length;
                  this.cantidad_admitidos = this.Aspirantes.filter(
                    (aspirante: any) => aspirante.Estado == "ADMITIDO"
                  ).length;
  ...
  loadPerfil(event: any) {
    ...
    this.tercerosService
      ...
      .subscribe(
        (response: any) => {
          this.projectService
            .get("proyecto_academico_institucion/" + this.proyectos_selected)
            .subscribe(
              (res: any) => {
                this.proyecto = res;
                this.inscripcion_id = event.data["Credencial"];
                this.inscripcionService
                  .get("inscripcion?query=Id:" + this.inscripcion_id)
                  .subscribe(
                    // (resp: any[]) => {
                    (resp: any = []) => {
                      ...
                      this.evaluacionInscripcionService
                        ...
                        .subscribe(
                          (respSuite: any) => {
                            if (
                              respSuite != null &&
                              respSuite.Status == "200"
                            ) {
                              if (Object.keys(respSuite.Data[0]).length > 0) {
                                this.tagsObject = JSON.parse(
                                  respSuite.Data[0].ListaTags
                                );

                                let TercerosAsociadoInscripcion = 0;
                                for (let i = 0; i < response.length; i++) {
                                  if (
                                    this.inscripcionInfo.PersonaId ==
                                    response[i].TerceroId.Id
                                  ) {
                                    TercerosAsociadoInscripcion =
                                      response[i].TerceroId.Id;
                                    break;
                                  }
                                }
                                ...
                                sessionStorage.setItem(
                                  "TerceroId",
                                  TercerosAsociadoInscripcion.toString()
                                );
                                sessionStorage.setItem(
                                  "IdInscripcion",
                                  event.data["Credencial"]
                                );
                                sessionStorage.setItem(
                                  "ProgramaAcademicoId",
                                  this.proyectos_selected.toString()
      ...
      if (data) {
        this.documentoService.get("documento/" + doc.DocumentoId).subscribe(
          ...
          (documento: any) => {
            //this.showProfile = true
            let metadataJoin = {
              ...JSON.parse(documento.Metadatos),
              ...data.metadata,
            };
        ...
        .subscribe(
          (resp: any) => {
            if (resp.Status == "200") {
              ...
              info.CorreoAlterno != ""
                ? correos.push(info.CorreoAlterno)
                : null;
                  ...
                  .subscribe(
                    (response: any) => {
                      Swal.fire({
                        ...
                        text: this.translate.instant(
                          "admision.desc_notificacion_success"
                        ),
                        confirmButtonText:
                    ...
                    (err) => {
                      Swal.fire({
                        icon: "error",
                        title: this.translate.instant(
                          "admision.titulo_notificacion_error"
                        ),
                        text: this.translate.instant(
                          "admision.desc_notificacion_error"
                        ),
                        confirmButtonText:
                          this.translate.instant("GLOBAL.aceptar"),
                      });
  ...
  getNombreRevisor() {
    this.tercerosService
      .get("tercero?query=Id:" + this.userService.getPersonaId())
      .subscribe(
        (res: any) => {
          // if (res !== null) {
  ...
  getTelefonoDependencia() {
    var telefono = "";
    this.oikosService
      .get("dependencia?query=id:" + this.proyecto.DependenciaId)
      .subscribe(
        (res: any) => {
          // if (res !== null) {
          if (res.length != 0) {
